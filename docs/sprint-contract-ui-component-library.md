...
- **In Scope**:
  1. 8 个基础组件: Button / Input / Select / FormField / Modal / Tooltip / Tree / PropertyPanel
  2. lib 工具: cn() (clsx + tailwind-merge)
  3. 构建链: tsup (已配置) + vitest 测试 + typecheck
  4. 测试: 每个组件 smoke test (渲染/交互/className 合并)
- **Out of Scope**: web 应用大规模迁移到新组件库（后续迭代）、VS Code 扩展接入（后续）

## Architecture Decision

- **技术栈**: React 18+ + TypeScript + CVA (class-variance-authority) + clsx + tailwind-merge
  （与 web 应用现有工具链一致，shadcn 风格）
- **样式策略**: 组件类名直接引用 shadcn HSL tokens (border/input/ring/card) 与
  app-* tokens (app-bg/app-text/app-border)，**不引入 tailwind 构建**——
  由消费方 (web app) 的 tailwind 提供这些类，组件库只输出 className 字符串
  （这是共享组件库的标准做法: UI 包不打包 CSS，样式由宿主应用解析）
- **依赖**: react, clsx, tailwind-merge, class-variance-authority, lucide-react (图标)
