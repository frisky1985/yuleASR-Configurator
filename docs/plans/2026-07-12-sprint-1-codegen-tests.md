...
> **周期:** 2026-07-13 ~ 2026-07-26（2周） **目标:**
> 把 Configurator 从"Schema 编辑器"变成能产出有效 C 代码的工具 **原则:** TDD
> — 先写测试再写实现，每个任务都是 RED → GREEN → REFACTOR

---

## Task 1: Generator 接口测试覆盖（Core 层）

**Objective:** 给 `CodeGenerator`
接口和基础工具函数写单元测试，建立生成器的测试基础设施
...
**Objective:**
验证 EcucCodeGenerator 的 validateConfig、generateHeaderFile 核心方法

**Files:**

- Create:
  `packages/@yuletech/core/src/generator/__tests__/ecuc-generator.test.ts`
- Modify: 无
    ...
    parameters: [
      {
        ...
        type: 'integer',
        required: true,
        description: 'CAN baud rate',
      },
      {
        name: 'canDevErrorDetect',
        type: 'boolean',
        required: false,
        description: 'DET enable',
      },
    ],
    containers: [],
  };
...
- Modify:
  `packages/@yuletech/core/src/generator/__tests__/ecuc-generator.test.ts`（追加）
...
- Create:
  `packages/@yuletech/core/src/generator/__tests__/rte-generator.test.ts`
...
describe('RteCodeGenerator', () => {
  ...
  it('should generate correct Rte.h guard', async () => {
    ...
    const rteH =
      result.files.find(f => f.path.endsWith('/Rte.h'))?.content || '';
  ...
  it('should parse RteInterfaces parameter correctly', async () => {
    ...
    const rteC =
      result.files.find(f => f.path.endsWith('/Rte.c'))?.content || '';
...
describe('Codegen - Module header generation', () => {
  ...
  it('should generate Adc_Cfg.h', () => {
    const module: ConfigModule = {
      ...
      version: '44',
      containers: [
        makeContainer('adcconfigset', 'ConfigSet', [
  ...
  it('should generate Can_Cfg.h with controller config', () => {
    const module: ConfigModule = {
      ...
      version: '80',
      containers: [
        makeContainer('canconfigset', 'ConfigSet', [
  ...
  it('should generate Mcu_Cfg.h with clock settings', () => {
    const module: ConfigModule = {
      ...
      version: '43',
      containers: [
        makeContainer('mcuclocksetting', 'ClockSetting', [
  ...
  it('should generate Port_Cfg.h with pin config', () => {
    const module: ConfigModule = {
      ...
      version: '42',
      containers: [
        makeContainer('portconfigset', 'ConfigSet', [
...
**Objective:**
将 ecuc-generator 和 rte-generator 公用的 C 代码格式化逻辑提取到工具函数
...
- `toGuardName(${moduleName.toUpperCase()}_H)` → 使用
  `ECUC_${moduleName.toUpperCase()}_H` 字面量即可（无需提取，已足够简单）
