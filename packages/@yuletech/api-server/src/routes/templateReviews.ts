...
export async function templateReviewsRoutes(app: FastifyInstance) {
  ...
  app.post('/bsw-templates/:templateId/reviews', { onRequest: [(app as any).authenticate] }, async (request, reply) => {
    ...
    if (existing) {
      throw { statusCode: 409, message: 'You have already reviewed this template. Use PUT to update.' }
    }
  ...
  app.put('/template-reviews/:id', { onRequest: [(app as any).authenticate] }, async (request, reply) => {
