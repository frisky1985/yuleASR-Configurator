...
export async function qaRoutes(app: FastifyInstance) {
  ...
  app.post('/answers/:id/accept', { onRequest: [(app as any).authenticate] }, async (request) => {
    ...
    if (answer.question.authorId !== user.id) throw { statusCode: 403, message: 'Only the question author can accept an answer' }
    if (answer.question.status === 'closed') throw { statusCode: 400, message: 'Question is closed' }
