...
export async function createPost(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (body.tags && body.tags.length > 0) {
    for (const tagName of body.tags) {
      ...
      if (existingTag) {
        await db.update(tags).set({ postCount: sql`${tags.postCount} + 1` }).where(eq(tags.id, existingTag.id))
      } else {
...
export async function updatePost(request: FastifyRequest, reply: FastifyReply) {
  ...
  const [existing] = await db.select().from(posts).where(eq(posts.id, parseInt(id))).limit(1)
  ...
  if (body.tags && existing.tags && existing.tags.length > 0) {
    for (const tagName of existing.tags) {
      const [existingTag] = await db.select().from(tags).where(eq(tags.name, tagName)).limit(1)
      if (existingTag) {
        await db.update(tags).set({ postCount: sql`GREATEST(${tags.postCount} - 1, 0)` }).where(eq(tags.id, existingTag.id))
      }
    }
  }
...
export async function listTags(_request: FastifyRequest, reply: FastifyReply) {
  ...
  }).from(tags).where(sql`${tags.postCount} > 0`).orderBy(desc(tags.postCount))
