/**
 * @yuletech/core - ConfigValidator (legacy)
 *
 * @deprecated 请使用 `@yuletech/core/validators` 下的验证管道
 * (ValidationPipeline / YuleasrValidator / CrossModuleValidator)。
 * 本实现仅保留供 yuleasr-editor-core 等存量调用方兼容使用，
 * 新代码请勿引用。入口 `@yuletech/core/validator` 同样标记为 deprecated。
 */
 ...
 * 配置验证器
 *
 * @deprecated 使用 `@yuletech/core/validators` 的验证管道替代。
 */
