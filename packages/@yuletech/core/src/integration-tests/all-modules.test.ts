...
describe('yuleASR-Configurator Integration Suite', () => {
  ...
  beforeAll(async () => {
    ...
    const genModule = await import('../generator/ecuc-generator.ts');
    ...
    for (const def of MODULE_DEFINITIONS) {
      it(`${def.module}: generates 4 ECUC files`, async () => {
        ...
        for (const f of filenames) {
          expect(result.files.some((rf: any) => rf.path === f || rf.path?.endsWith(f))).toBe(true);
        }
      });
    }
  });
    ...
    for (const def of MODULE_DEFINITIONS) {
      it(`${def.module}: all ECUC files pass syntax check`, async () => {
        ...
        for (const f of result.files) {
          const filePath = join(tempDir, def.module, 'syntax', basename(f.path));
          writeFileSync(filePath, f.content);
          const ok = gccSyntaxCheck(filePath, [stubDir, moduleDir]);
          if (!ok) {
            allPass = false;
            failures.push(basename(f.path));
          }
        }
  ...
  describe('C1 — AUTOSAR Compliance', () => {
    for (const def of MODULE_DEFINITIONS) {
      it(`${def.module}: generated files contain expected AUTOSAR elements`, async () => {
        ...
        const cfgFile = result.files.find((f: any) => f.path.endsWith('Cfg.h'));
    ...
    for (const def of MODULE_DEFINITIONS) {
      it(`${def.module}: two generations produce identical output`, async () => {
        ...
        for (let i = 0; i < r1.files.length; i++) {
          // Strip timestamp lines which naturally differ between runs
          const stripDate = (s: string) => s.replace(/@date\s+.*/g, '@date [STRIPPED]');
          const content1 = stripDate(r1.files[i].content);
          const content2 = stripDate(r2.files[i].content);
