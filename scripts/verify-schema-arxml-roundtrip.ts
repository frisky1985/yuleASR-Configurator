...
const DEFAULT_REPORT =
  '/Users/stefan/.openclaw/workspace/reports/schema-to-arxml-roundtrip-20260812.md';
...
function main(): void {
  ...
  const files = readdirSync(arxmlDir)
  ...
  for (const f of files) {
    ...
    for (const def of parsed.ecucModuleDefs) {
      ...
      const schemaParamsByKey = new Map<
        string,
        { cls: GeneratedModule['moduleParams'][number]['cls']; container: string | null }
      >();
      for (const p of expected.moduleParams)
        schemaParamsByKey.set(`<module>/${p.name}`, { cls: p.cls, container: null });
      for (const c of expected.containers) {
        for (const p of c.params)
          schemaParamsByKey.set(`${c.name}/${p.name}`, { cls: p.cls, container: c.name });
      }
      const allDefs = [
        ...def.parameterDefs.map(p => ({ p, container: null as string | null })),
        ...def.containerDefs.flatMap(c =>
          c.parameterDefs.map(p => ({ p, container: c.name as string | null }))
        ),
      ];
  ...
  const modulesMatched = generated.filter(m => {
    ...
    return (
      ...
      def.containerDefs.every(c => {
        ...
        return (
          exp !== undefined &&
          sortedSet(c.parameterDefs.map(p => p.name)).join(',') === sortedSet(exp).join(',')
        );
      })
    );
  }).length;
  ...
  const totalParams = generated.reduce(
    (n, m) => n + m.moduleParams.length + m.containers.reduce((k, c) => k + c.params.length, 0),
    0
  );
  const totalSkipped = generated.reduce((n, m) => n + m.skipped.length, 0);
  const structDiffs = diffs.filter(
    d => d.level === 'module' || d.level === 'container' || d.level === 'param'
  );
  ...
  lines.push(
    `- 解析器：\`packages/@yuletech/core/src/arxml-import/reader.ts\`（parseSwcArxml / EcucModuleDef，R8/E2）`
  );
  ...
  lines.push('|---|---|---|');
  lines.push(
    '| schema 文件（117 个） | `<ECUC-MODULE-DEF>` | 模块名 = title 去 ` Configuration` 后缀 |'
  );
  lines.push('| 顶层非 object 属性 | `<PARAMETER-DEFS>` | 模块级参数定义 |');
  lines.push(
    '| 顶层 object 属性 | `<CONTAINER-DEFS><ECUC-CONTAINER-DEF>` | 容器定义（含参数定义） |'
  );
  lines.push('| `boolean` | `ECUC-BOOLEAN-PARAM-DEF` | — |');
  lines.push(
    '| `integer`（含 integer+enum） | `ECUC-INTEGER-PARAM-DEF` | 带 MIN/MAX（schema 有值时） |'
  );
  lines.push('| `number` | `ECUC-FLOAT-PARAM-DEF` | — |');
  lines.push('| `string`（无 enum） | `ECUC-STRING-PARAM-DEF` | — |');
  lines.push(
    '| `string`+enum / `enum` | `ECUC-ENUMERATION-PARAM-DEF` + `LITERALS` | enum 值 → literal 短名 |'
  );
  lines.push('| `reference` | `ECUC-REFERENCE-DEF` | 回调指针类参数 |');
  lines.push('| `array` / 其他 | 跳过 | ECUC-PARAM-DEF 族不可表达（记录 skipped 清单） |');
  lines.push(
    '| multiplicity 无信息 | 参数 0..1；容器 0..-1（*） | 显式补默认，避免 reader 隐式推断 |'
  );
  lines.push('');
  lines.push('## 2. 覆盖规模');
  lines.push('');
  ...
  lines.push(
    `- 跳过（不可表达）：**${totalSkipped}**（array 类型为主，含 appswc/compswc 的 SWC 结构数组）`
  );
  lines.push('');
  ...
  lines.push(
    `- 结构完全一致模块（模块级参数 + 容器 + 每容器参数名集合全部相等）：**${modulesMatched} / ${generated.length}（${((modulesMatched / generated.length) * 100).toFixed(1)}%）**`
  );
  ...
  lines.push(
    `- 解析告警总数：**${totalWarnings}**（${[...warningTagCounts.entries()].map(([k, v]) => `${k}:${v}`).join('，') || '无'}）`
  );
  ...
  lines.push('');
  lines.push(
    '1. **multiplicity 无信息默认值**：schema 不含 multiplicity 信息，生成侧显式补 0..1（参数）/ 0..-1（容器）；reader 只记录显式值，故解析结果与生成一致，不构成结构差异。'
  );
  lines.push(
    '2. **enum 无 LITERALS 表达**：`integer`+enum（McuResetReason / OsIsrCategory / SpiDataWidth）按主类型 `ECUC-INTEGER-PARAM-DEF` 生成，enum 符号常量语义丢失（reader 的 integer 定义也不捕获 LITERALS，双向一致）；若需符号常量，应补 COMPU-METHOD（超出本方案范围）。'
  );
  lines.push(
    '3. **array 类型跳过**：ECUC-PARAM-DEF 族无数组定义（AUTOSAR 定义层用容器 + 多实例表达）；appswc/compswc 的 SWC 结构数组与 OsScheduleTableExpiryPoints 等 17 个 array 参数被跳过并计数，不参与结构对比。'
  );
  lines.push(
    '4. **MIN/MAX 未建模告警**：生成侧输出 MIN/MAX（integer 参数 3221/3222 个有界），reader R8 边界未建模 MIN/MAX → 每条产生一条 Unprocessed 告警（不崩溃、不影响结构）；若需零告警可加 `--no-min-max` 重新生成。'
  );
  lines.push(
    '5. **reference 参数**：9 个回调指针（doip 等）映射 `ECUC-REFERENCE-DEF`，reader 归一 kind=REFERENCE，一致。'
  );
  lines.push('');
  lines.push('## 5. 结论');
  lines.push('');
  if (structDiffs.length === 0 && parsedModuleCount === generated.length) {
    lines.push(
      `**通过**：schema → ARXML → 解析的结构闭环 100% 一致（${generated.length}/${generated.length} 模块、${totalContainers} 容器、${totalParams} 参数名集合全部吻合），类型映射与枚举 LITERALS 零差异。`
    );
    lines.push('');
    lines.push(`反向生成的 ECUC-MODULE-DEF ARXML **可作为"标准模板"替代丢失的源 ARXML**：`);
    lines.push(
      '- 结构层（模块/容器/参数名 + 类型归一）由修复后 schema 完全决定，round-trip 验证了 schema 修复与模板生成的一致性；'
    );
    lines.push(
      '- 保真度缺口仅剩：MIN/MAX（生成已有，reader 未建模）、array 参数（17 个，不可表达）、integer 枚举符号常量（3 个，需 COMPU-METHOD）。'
    );
    lines.push(
      '- 建议：将 `/tmp/ecuc-defs-generated/` 产物纳入版本管理或按需合并为单文件，作为 E3 一致性校验与定义↔值关联的标准定义层输入。'
    );
  } else {
    lines.push(
      `**未完全通过**：存在 ${structDiffs.length} 条结构差异 / ${parsedModuleCount}/${generated.length} 模块解析成功，需先修复生成映射再作为标准模板。`
    );
  }
  lines.push('');
  lines.push('## 6. 复现命令');
  ...
  console.log(
    `   模块: ${parsedModuleCount}/${generated.length} 解析成功；结构一致 ${modulesMatched}/${generated.length}（${((modulesMatched / generated.length) * 100).toFixed(1)}%）`
  );
  console.log(`   容器: ${totalContainers}，参数: ${totalParams}，跳过: ${totalSkipped}`);
  console.log(
    `   结构差异: ${structDiffs.length}，类型差异: ${diffs.filter(d => d.level === 'kind').length}，LITERALS 差异: ${diffs.filter(d => d.level === 'literal').length}`
  );
  console.log(
    `   告警: ${totalWarnings}（${[...warningTagCounts.entries()].map(([k, v]) => `${k}:${v}`).join('，')}），错误: ${totalErrors}`
  );
