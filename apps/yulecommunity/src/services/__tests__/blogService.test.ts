...
import { articleService, tagService, commentService } from '../blogService';
...
describe('articleService', () => {
  ...
  describe('getArticles', () => {
    ...
    it('应该按标签过滤文章', async () => {
      ...
      expect(
        result.data.every(article => article.tags.some(t => t.toLowerCase() === 'autosar'))
      ).toBe(true);
    });
...
describe('tagService', () => {
  ...
  describe('getArticlesByTag', () => {
    it('应该返回指定标签的文章', async () => {
      ...
      expect(articles.every(article => article.tags.some(t => t.toLowerCase() === 'autosar'))).toBe(
        true
      );
    });
