...
export const DIO_APIS: AutosarApi[] = [
  {
    ...
    description:
      '返回指定 DIO 通道的当前电平状态。该函数直接读取 GPIO 输入寄存器，具有极低的调用开销，适用于时序敏感的应用场景。',
    params: [
      {
        name: 'ChannelId',
        type: 'Dio_ChannelType',
        direction: 'in',
        description: 'DIO 通道标识符，标识要读取的引脚',
      },
    ],
...
void Dio_ReadButton(void) {
  ...
  },
  {
    ...
    description:
      '将指定 DIO 通道的电平设置为 STD_HIGH 或 STD_LOW。该函数直接写入 GPIO 输出寄存器，适用于控制 LED、继电器等输出设备。',
    params: [
      {
        name: 'ChannelId',
        type: 'Dio_ChannelType',
        direction: 'in',
        description: 'DIO 通道标识符',
      },
      {
        name: 'Level',
        type: 'Dio_LevelType',
        direction: 'in',
        description: '目标电平：STD_HIGH 或 STD_LOW',
      },
    ],
    returnType: 'void',
...
void Dio_ControlLed(void) {
  ...
  },
  {
    ...
    description:
      '将指定 DIO 通道的当前电平翻转（高→低或低→高），并返回翻转后的电平值。该操作是原子性的，适用于不需要关心当前状态的应用场景。',
    params: [
      {
        name: 'ChannelId',
        type: 'Dio_ChannelType',
...
void Dio_ToggleLed(void) {
  ...
  },
  {
    ...
    description:
      '一次性读取指定 DIO 端口所有引脚的电平状态。返回值是一个位掩码，每个位对应一个引脚的电平。适用于需要快速读取一组引脚状态的场景。',
    params: [
...
void Dio_ReadAllPins(void) {
  ...
  },
  {
    ...
    description:
      '一次性设置指定 DIO 端口所有引脚的电平状态。Level 参数是一个位掩码，每个位对应一个引脚的电平。未使用的位应设置为 0。',
    params: [
      { name: 'PortId', type: 'Dio_PortType', direction: 'in', description: 'DIO 端口标识符' },
      {
        name: 'Level',
...
void Dio_SetPortOutput(void) {
  ...
  },
  {
    ...
    brief: '读取指定通道组的电平状态',
    description:
      '读取指定通道组中所有引脚的电平状态。通道组是跨端口的一组连续引脚的逻辑分组，返回值仅包含该组引脚的掩码位。',
    params: [
      {
        name: 'ChannelGroupIdPtr',
        type: 'const Dio_ChannelGroupType*',
        direction: 'in',
        description: '通道组配置指针，包含端口基准、偏移和掩码信息',
      },
    ],
    returnType: 'Dio_PortLevelType',
...
void Dio_ReadLedGroup(void) {
  ...
  },
  {
    ...
    brief: '设置指定通道组的电平状态',
    description:
      '设置指定通道组中所有引脚的电平状态。仅修改通道组掩码覆盖的引脚，不影响该端口中其他引脚的状态。',
    params: [
      {
        name: 'ChannelGroupIdPtr',
        type: 'const Dio_ChannelGroupType*',
...
void Dio_SetLedPattern(uint8 pattern) {
  ...
  },
  {
    ...
    description:
      '返回 Dio 驱动模块的厂商 ID、模块 ID、软件版本号等版本信息。用于诊断和兼容性检查。',
    params: [
      {
        name: 'VersionInfo',
...
void Dio_PrintVersion(void) {
  ...
  },
  {
    ...
    signature:
      'Std_ReturnType Dio_SetChannelMode(Dio_ChannelType ChannelId, Dio_ChannelModeType Mode)',
    brief: '设置 DIO 通道的工作模式',
    description:
      '在运行时动态改变指定 DIO 通道的工作模式，如推挽输出、开漏输出、高阻输入等。该函数为可选功能，并非所有硬件平台都支持。',
    params: [
      {
        name: 'ChannelId',
        type: 'Dio_ChannelType',
      ...
      {
        name: 'Mode',
        type: 'Dio_ChannelModeType',
        direction: 'in',
        description: '目标工作模式（推挽/开漏/高阻等）',
      },
    ],
...
void Dio_SwitchToOpenDrain(void) {
  ...
  },
  {
    ...
    description:
      '读取指定 DIO 通道的输出锁存寄存器值，而非实际的引脚电平。该函数用于确认已写入的输出值，与 Dio_ReadChannel（读取实际引脚电平）不同。',
    params: [
      {
        name: 'ChannelId',
        type: 'Dio_ChannelType',
