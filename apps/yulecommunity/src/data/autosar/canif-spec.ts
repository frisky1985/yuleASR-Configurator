...
export const CANIF_APIS: AutosarApi[] = [
  {
    ...
    brief: '初始化 CanIf 模块',
    description:
      '初始化 CAN 接口模块，配置 PDU 路由、控制器句柄、硬件对象过滤等参数。必须在任何其他 CanIf 操作前调用。',
    params: [
      {
        name: 'ConfigPtr',
        type: 'const CanIf_ConfigType*',
        direction: 'in',
        description: 'CanIf 配置指针，包含所有 PDU 和控制器配置',
      },
    ],
    returnType: 'void',
...
void CanIf_InitExample(void) {
    ...
    configParams: [{ paramName: 'CanIfConfig', configModule: 'CanIf', path: 'CanIf/CanIfConfig' }],
    status: 'standard',
  },
  {
    id: 'CanIf_DeInit',
    ...
    brief: '反初始化 CanIf 模块',
    description:
      '反初始化 CanIf 模块，释放所有内部资源。调用后 CanIf 恢复到未初始化状态，所有 PDU 收发功能不可用。',
    params: [],
    returnType: 'void',
    returnDescription: '无返回值',
    moduleId: 'CanIf',
...
void CanIf_PrintVersion(void) {
  ...
  },
  {
    ...
    signature:
      'Std_ReturnType CanIf_SetBaudrate(CanIf_ControllerRefType ControllerId, uint16 BaudRateConfigID)',
    brief: '设置 CAN 控制器波特率',
    description:
      '通过 CanIf 设置指定 CAN 控制器的通信波特率。CanIf 将请求转发到 Can 驱动层执行实际的波特率配置。',
    params: [
      {
        name: 'ControllerId',
        type: 'CanIf_ControllerRefType',
        direction: 'in',
        description: 'CAN 控制器引用 ID',
      },
...
void CanIf_ChangeBaudrate(void) {
  ...
  },
  {
    ...
    brief: '发送 CAN PDU',
    description:
      '通过 CanIf 发送指定的 CAN PDU。CanIf 根据 PDU ID 查找对应的硬件句柄和控制器，调用 Can 驱动层发送。支持确认和取消操作。',
    params: [
      {
        name: 'CanTxPduId',
        type: 'uint32',
        direction: 'in',
        description: '发送 PDU ID，标识要发送的 CAN 消息',
      },
      {
        name: 'PduInfoPtr',
        type: 'const PduInfoType*',
        direction: 'in',
        description: 'PDU 数据指针，包含 SDU 数据、长度等',
      },
    ],
    returnType: 'Std_ReturnType',
...
void CanIf_SendMessage(void) {
  ...
  },
  {
    ...
    description:
      '取消之前提交的指定 CAN PDU 的发送请求。如果该 PDU 尚未被硬件发送，将取消发送队列中的请求。',
    params: [
...
void CanIf_AbortTx(void) {
  ...
  },
  {
    ...
    description:
      '从 CanIf 接收缓冲区读取指定接收 PDU 的最新数据。通常由上层模块（如 Com、PduR）在接收到 RxIndication 后调用。',
    params: [
...
void CanIf_ReadReceivedData(void) {
  ...
  },
  {
    ...
    brief: '设置 PDU 通信模式',
    description:
      '设置指定 PDU 的通信模式，支持 CANIF_OFFLINE（离线）、CANIF_TX_ONLY（只发）、CANIF_RX_ONLY（只收）、CANIF_ONLINE（在线）等模式。',
    params: [
...
void CanIf_SetTxOnly(void) {
  ...
  },
  {
    ...
    params: [{ name: 'CanPduId', type: 'uint32', direction: 'in', description: 'PDU ID' }],
...
void CanIf_CheckPduMode(void) {
  ...
  },
  {
    ...
    brief: '发送确认回调函数',
    description:
      'Can 驱动在 CAN 消息发送完成后调用的回调函数。CanIf 根据结果通知上层模块（如 Com、PduR）发送完成状态。',
    params: [
      { name: 'CanTxPduId', type: 'uint32', direction: 'in', description: '已发送的 PDU ID' },
      {
        ...
        type: 'Std_ReturnType',
        direction: 'in',
        description: '发送结果：E_OK 表示成功，E_NOT_OK 表示失败',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Can_Driver_TxDone(uint32 CanTxPduId, Std_ReturnType result) {
  ...
  },
  {
    ...
    brief: '接收指示回调函数',
    description:
      'Can 驱动在接收到 CAN 消息后调用的回调函数。CanIf 将接收到的 PDU 路由到上层模块（如 Com、PduR）进行处理。',
    params: [
...
void Can_Driver_RxDone(uint32 CanRxPduId, const PduInfoType* pduInfo) {
  ...
  },
  {
    ...
    signature:
      'void CanIf_ControllerModeIndication(CanIf_ControllerRefType ControllerId, Can_ControllerStateType ControllerState)',
    brief: '控制器模式改变指示回调',
    description:
      'Can 驱动在 CAN 控制器状态改变时调用的回调函数。通知 CanIf 控制器进入 STARTED、STOPPED 或 SLEEP 等模式。',
    params: [
      {
        name: 'ControllerId',
        type: 'CanIf_ControllerRefType',
