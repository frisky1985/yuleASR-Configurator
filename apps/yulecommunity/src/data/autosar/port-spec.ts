...
export const PORT_APIS: AutosarApi[] = [
  {
    ...
    description:
      '初始化 Port 模块，根据配置表一次性设置所有引脚的功能复用、方向、驱动能力和上下拉电阻。必须在任何 Port 引脚操作前调用。',
    params: [
      {
        name: 'ConfigPtr',
        type: 'const Port_ConfigType*',
        direction: 'in',
        description: 'Port 配置表指针，包含所有引脚的初始化配置',
      },
    ],
    returnType: 'void',
...
void Port_InitExample(void) {
  ...
  },
  {
    ...
    brief: '运行时设置引脚方向',
    description:
      '在运行时动态改变指定引脚的方向（输入或输出）。适用于需要在运行时切换引脚方向的场景，如双向数据总线。',
    params: [
      { name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' },
      {
        ...
        direction: 'in',
        description: '目标方向：PORT_PIN_IN 或 PORT_PIN_OUT',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Port_SwitchDirection(void) {
  ...
  },
  {
    ...
    brief: '刷新所有引脚方向为配置值',
    description:
      '将所有引脚的方向刷新为配置表中的初始值。用于在运行时恢复引脚方向到已知状态，例如从错误恢复或模式切换后。',
    params: [],
    returnType: 'void',
    returnDescription: '无返回值',
    moduleId: 'Port',
...
void Port_ResetDirections(void) {
  ...
  },
  {
    ...
    brief: '设置引脚功能复用模式',
    description:
      '将指定引脚切换到不同的功能复用模式，如 GPIO、UART、CAN、SPI 等。必须在引脚锁定前调用，锁定后调用无效。',
    params: [
      { name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' },
      {
        name: 'Mode',
        ...
        direction: 'in',
        description: '目标复用模式（如 PORT_PIN_MODE_DIO、PORT_PIN_MODE_CAN）',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Port_PrintVersion(void) {
  ...
  },
  {
    ...
    brief: '设置引脚内部上下拉电阻',
    description:
      '配置指定引脚的内部上下拉电阻，支持上拉、下拉或浮空三种状态。用于确保输入引脚在悬空时处于确定电平。',
    params: [
      { name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' },
      {
        name: 'PullConfig',
        type: 'Port_PinPullConfigType',
        direction: 'in',
        description: '上下拉配置：上拉/下拉/浮空',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Port_EnablePullUp(void) {
  ...
  },
  {
    ...
    brief: '设置引脚输出压摆率',
    description:
      '配置输出引脚的信号压摆率。高速通信接口（如 CAN、SPI）通常需要高压摆率，而普通 GPIO 可使用低压摆率以降低 EMI。',
    params: [
      { name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' },
      {
        name: 'SlewRate',
        type: 'Port_PinSlewRateType',
        direction: 'in',
        description: '压摆率配置：低/中/高',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Port_FastOutput(void) {
  ...
  },
  {
    ...
    signature:
      'void Port_SetPinDriveStrength(Port_PinType Pin, Port_PinDriveStrengthType Strength)',
    brief: '设置引脚驱动强度',
    description:
      '配置输出引脚的驱动能力。高驱动强度适用于大负载或高速开关场景，低驱动强度有助于降低功耗和 EMI。',
    params: [
      { name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' },
      {
        name: 'Strength',
        type: 'Port_PinDriveStrengthType',
        direction: 'in',
        description: '驱动强度：低/中/高',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Port_StrongDrive(void) {
  ...
  },
  {
    ...
    brief: '设置引脚输入滤波器',
    description:
      '配置输入引脚的数字滤波器，可滤除短于设定宽度的噪声脉冲。适用于按键、开关等易产生抖动的输入信号。',
    params: [
      { name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' },
      {
        name: 'Filter',
        type: 'Port_PinInputFilterType',
        direction: 'in',
        description: '滤波器配置：旁路/单周期/多周期',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Port_EnableDebounce(void) {
  ...
  },
  {
    ...
    brief: '锁定引脚配置防止意外修改',
    description:
      '锁定指定引脚的配置，防止后续误操作修改引脚的功能复用、方向或电气特性。锁定后，对该引脚的大多数配置函数调用将被忽略。通常用于关键安全引脚的配置保护。',
    params: [
...
void Port_ProtectPins(void) {
  ...
  },
  {
    ...
    brief: '获取引脚当前模式',
    description:
      '读取指定引脚当前的功能复用模式。用于诊断或调试目的，确认引脚已被正确配置为目标模式。',
    params: [{ name: 'Pin', type: 'Port_PinType', direction: 'in', description: '引脚 ID' }],
