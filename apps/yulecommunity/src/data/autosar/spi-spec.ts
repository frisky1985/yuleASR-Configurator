...
export const SPI_APIS: AutosarApi[] = [
  {
    ...
    brief: '初始化 SPI 驱动模块',
    description:
      '初始化 SPI 驱动模块，配置通道、序列、作业和传输模式。必须在任何 SPI 操作前调用，且只能调用一次。',
    params: [
      {
        name: 'ConfigPtr',
        type: 'const Spi_ConfigType*',
        direction: 'in',
        description: 'SPI 配置表指针，包含所有通道和作业配置',
      },
    ],
    returnType: 'void',
...
void Spi_Shutdown(void) {
  ...
  },
  {
    ...
    brief: '向通道写入数据到内部缓冲区',
    description:
      '将数据写入指定通道的内部缓冲区。数据在 Spi_AsyncTransmit 或同步传输时发送到外设。支持跨作业的数据传输。',
    params: [
      { name: 'Channel', type: 'Spi_ChannelType', direction: 'in', description: '目标通道 ID' },
      {
        name: 'DataBufferPtr',
        type: 'const Spi_DataBufferType*',
        direction: 'in',
        description: '指向待发送数据的缓冲区指针',
      },
    ],
...
void Spi_ReadReceived(void) {
  ...
  },
  {
    ...
    brief: '启动异步 SPI 传输序列',
    description:
      '以非阻塞方式启动指定序列的 SPI 传输。传输完成后通过回调函数通知上层。传输期间 CPU 可执行其他任务。',
    params: [
      {
        name: 'Sequence',
        type: 'Spi_SequenceType',
        direction: 'in',
        description: '要执行的序列 ID',
      },
    ],
    returnType: 'Std_ReturnType',
...
void Spi_StartTransfer(void) {
  ...
  },
  {
    ...
    brief: '启动同步 SPI 传输序列',
    description:
      '以阻塞方式启动指定序列的 SPI 传输。函数在传输完成后返回，期间 CPU 忙等待或阻塞。适用于短数据传输或时序关键场景。',
    params: [
      {
        name: 'Sequence',
        type: 'Spi_SequenceType',
...
void Spi_ReadRegister(uint8 regAddr) {
  ...
  },
  {
    ...
    brief: '配置外部缓冲区用于数据传输',
    description:
      '将外部缓冲区绑定到指定通道。启用外部缓冲区后，SPI 数据直接从外部缓冲区读写，绕过内部缓冲区，适用于大数据块传输。',
    params: [
      { name: 'Channel', type: 'Spi_ChannelType', direction: 'in', description: '目标通道 ID' },
      {
        name: 'EBConfigPtr',
        type: 'const Spi_EBType*',
        direction: 'in',
        description: '外部缓冲区配置指针',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Spi_SetupExtBuf(void) {
  ...
  },
  {
    ...
    brief: '获取 SPI 驱动模块状态',
    description:
      '返回 SPI 驱动模块的当前状态：SPI_UNINIT（未初始化）、SPI_IDLE（空闲）、SPI_BUSY（忙）。',
    params: [],
...
void Spi_CheckReady(void) {
  ...
  },
  {
    ...
    brief: '获取指定作业的执行结果',
    description:
      '返回指定 SPI 作业的执行状态：SPI_JOB_OK（成功）、SPI_JOB_PENDING（进行中）、SPI_JOB_FAILED（失败）等。',
    params: [{ name: 'Job', type: 'Spi_JobType', direction: 'in', description: '要查询的作业 ID' }],
...
void Spi_CheckSeq(void) {
  ...
  },
  {
    ...
    description:
      '取消指定 SPI 序列的执行。如果序列正在传输，将终止当前和后续所有作业。适用于超时或错误恢复场景。',
    params: [
      {
        name: 'Sequence',
        type: 'Spi_SequenceType',
        direction: 'in',
        description: '要取消的序列 ID',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
    ...
    params: [
      {
        ...
        direction: 'in',
        description: '异步模式：SPI_POLLING_MODE 或 SPI_INTERRUPT_MODE',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Spi_EnableInterrupt(void) {
  ...
  },
  {
    ...
    signature:
      'Std_ReturnType Spi_SetDataWidth(Spi_ChannelType Channel, Spi_DataWidthType DataWidth)',
    brief: '设置 SPI 数据宽度',
    description:
      '配置指定 SPI 通道的数据位宽（如 8 位、16 位、32 位）。不同的 SPI 外设可能要求不同的数据宽度。',
    params: [
      { name: 'Channel', type: 'Spi_ChannelType', direction: 'in', description: '目标通道 ID' },
      {
        name: 'DataWidth',
        type: 'Spi_DataWidthType',
        direction: 'in',
        description: '数据宽度（位）',
      },
    ],
...
void Spi_16BitMode(void) {
  ...
  },
  {
    ...
    params: [
      ...
      {
        name: 'BitRate',
        type: 'Spi_BitRateType',
        direction: 'in',
        description: '目标波特率（Hz）',
      },
    ],
    returnType: 'Std_ReturnType',
...
void Spi_SlowSpeed(void) {
  ...
  },
  {
    ...
    brief: '获取 SPI 硬件传输结果',
    description:
      '返回 SPI 硬件层面的传输结果，包括错误标志、超时状态等底层信息。用于诊断 SPI 通信故障。',
    params: [
      { name: 'Channel', type: 'Spi_ChannelType', direction: 'in', description: '目标通道 ID' },
    ],
...
void Spi_CheckHwError(void) {
  ...
  },
  {
    ...
    signature:
      'Std_ReturnType Spi_SetCsPolarity(Spi_ChannelType Channel, Spi_CsPolarityType Polarity)',
    brief: '设置片选信号极性',
    ...
    params: [
      ...
      {
        name: 'Polarity',
        type: 'Spi_CsPolarityType',
        direction: 'in',
        description: 'CS 极性：低电平有效或高电平有效',
      },
    ],
    returnType: 'Std_ReturnType',
...
void Spi_ConfigCs(void) {
  ...
  },
  {
    ...
    name: 'Spi_Exchange',
    signature:
      'Std_ReturnType Spi_Exchange(Spi_ChannelType Channel, Spi_DataBufferType* DataBufferPtr, uint16 Length)',
    brief: '全双工同步数据交换',
    description:
      '执行全双工 SPI 数据交换：在发送数据的同时接收数据。适用于需要同时发送和接收的场景，如读取 SPI 寄存器。',
    params: [
      { name: 'Channel', type: 'Spi_ChannelType', direction: 'in', description: '目标通道 ID' },
      {
        name: 'DataBufferPtr',
