...
export const ECUM_APIS: AutosarApi[] = [
  {
    ...
    brief: '初始化 ECU 状态管理器',
    description:
      '初始化 EcuM 模块。在系统上电后最早调用的 BSW 模块之一，负责建立基础的运行环境，初始化 BSW 调度器，并过渡到 RUN 状态。',
    params: [],
    returnType: 'void',
...
void Startup_Sequence(void) {
    ...
    configParams: [{ paramName: 'EcuMConfig', configModule: 'EcuM', path: 'EcuM/EcuMConfig' }],
    status: 'standard',
  },
  {
...
void EcuM_PrintVersion(void) {
  ...
  },
  {
    ...
    brief: '选择唤醒源',
    description:
      '选择一个或多个唤醒源用于 ECU 唤醒。在进入睡眠模式前调用，指定哪些外部事件可以唤醒 ECU，如 CAN 总线活动、LIN 唤醒、外部引脚电平变化等。',
    params: [
      {
        name: 'WakeupSource',
...
void EcuM_PrepareSleep(void) {
  ...
  },
  {
    ...
    brief: '检查指定唤醒源是否有效',
    description:
      '验证指定的唤醒源是否确实产生了唤醒事件。EcuM 会调用各通信模块的检查函数来确认唤醒源的有效性，防止误唤醒。',
    params: [
      {
        name: 'WakeupSource',
        type: 'EcuM_WakeupSourceType',
...
void EcuM_ValidateWakeup(void) {
  ...
  },
  {
    ...
    brief: '启动唤醒源监视',
    description:
      '启动指定唤醒源的监视功能。使能相应的通信控制器或 GPIO 中断，使其能够在总线活动或电平变化时唤醒 ECU。',
    params: [
      {
        name: 'WakeupSource',
        type: 'EcuM_WakeupSourceType',
...
void EcuM_EnableWakeupMonitors(void) {
  ...
  },
  {
    ...
    brief: '停止唤醒源监视',
    description:
      '停止指定唤醒源的监视功能。通常在 ECU 进入正常运行模式后调用，关闭唤醒监视以降低功耗。',
    params: [
      {
        name: 'WakeupSource',
        type: 'EcuM_WakeupSourceType',
...
void EcuM_DisableWakeupMonitors(void) {
  ...
  },
  {
    ...
    brief: '获取当前唤醒事件源',
    description:
      '返回触发 ECU 从睡眠模式唤醒的事件源。用于确定是哪个外设或条件触发了唤醒，以便执行对应的唤醒处理流程。',
    params: [],
...
void EcuM_IdentifyWakeupSource(void) {
  ...
  },
  {
    ...
    brief: '请求重置到 RUN 状态',
    description:
      '请求将 ECU 状态重置并过渡到 RUN 模式。在需要从错误恢复或模式切换时调用，EcuM 将执行重置序列并重新初始化系统到 RUN 状态。',
    params: [],
    returnType: 'void',
...
void EcuM_TriggerReset(void) {
  ...
  },
  {
    ...
    description:
      '请求将 ECU 从 RUN 模式过渡到 POSTRUN 模式。POSTRUN 模式下，部分 BSW 模块仍在运行但应用层已停止，用于执行关闭前清理操作。',
    params: [],
    returnType: 'void',
    returnDescription: '无返回值',
    moduleId: 'EcuM',
...
void EcuM_EnterPostRun(void) {
  ...
  },
  {
    ...
    brief: '设置唤醒事件',
    description:
      '由通信模块或中断服务程序调用，通知 EcuM 检测到指定唤醒源的事件。EcuM 收集所有唤醒事件并触发唤醒验证流程。',
    params: [
      {
        name: 'WakeupSource',
        type: 'EcuM_WakeupSourceType',
...
void Can_WakeupInterrupt(void) {
  ...
  },
  {
    ...
    brief: 'EcuM 主函数周期性调用',
    description:
      'EcuM 的周期性主函数。必须在操作系统的周期性任务中调用，用于 EcuM 的内部状态机处理、唤醒验证超时管理和模式转换协调。',
    params: [],
    returnType: 'void',
    returnDescription: '无返回值',
    moduleId: 'EcuM',
...
void EcuM_PeriodicTask(void) {
  ...
  },
  {
    ...
    brief: '验证并确认唤醒源',
    description:
      '对指定的唤醒源进行最终验证确认。在 EcuM_CheckWakeup 确认有效后调用，完成唤醒源的完整验证流程并将 ECU 过渡到唤醒状态。',
    params: [
      {
        name: 'WakeupSource',
        type: 'EcuM_WakeupSourceType',
        direction: 'in',
        description: '要验证确认的唤醒源',
      },
    ],
    returnType: 'Std_ReturnType',
