...
export const CAN_APIS: AutosarApi[] = [
  {
    ...
    description:
      '该函数初始化 CAN 控制器模块，包括 CAN 控制器的选择、波特率、唤醒方式、接收 ID 过滤规则等。必须在任何其他 CAN 操作之前调用。',
    params: [
      {
        name: 'Config',
        type: 'const Can_ConfigType*',
        direction: 'in',
        description: 'CAN 配置结构体指针，包含控制器和硬件对象配置',
      },
    ],
...
void Can_InitExample(void) {
  ...
  },
  {
    ...
    brief: '发送 CAN 消息到总线',
    description:
      '将给定的 CAN 消息（包含 ID、DLC、数据）发送到指定硬件句柄对应的 CAN 控制器。支持标准帧（11-bit ID）和扩展帧（29-bit ID）格式。',
    params: [
      {
        name: 'Hth',
        type: 'Can_HwHandleType',
        direction: 'in',
        description: 'CAN 硬件发送句柄，指定发送控制器',
      },
      {
        name: 'PduInfo',
        type: 'const Can_PduType*',
        direction: 'in',
        description: 'CAN 消息结构体，包含 ID、长度、数据载荷',
      },
    ],
    returnType: 'Std_ReturnType',
...
void Can_WriteExample(void) {
  ...
  },
  {
    ...
    brief: '读取接收到的 CAN 消息',
    description:
      '从接收缓冲器中读取已接收的 CAN 消息内容。通常从中断服务程序或轮询任务中调用。接收到的数据通过 PduInfo 指针返回。',
    params: [
      { name: 'Hrh', type: 'Can_HwHandleType', direction: 'in', description: 'CAN 硬件接收句柄' },
      {
        ...
        type: 'Can_PduType*',
        direction: 'out',
        description: '输出缓冲区，存放接收到的 CAN 消息',
      },
    ],
    returnType: 'void',
...
void Can_IrqHandler(void) {
  ...
  },
  {
    ...
    description:
      '在运行时动态改变指定 CAN 控制器的通信波特率。波特率配置 ID 必须在配置阶段预定义。',
    params: [
      { name: 'Hth', type: 'Can_HwHandleType', direction: 'in', description: 'CAN 硬件句柄' },
      {
        name: 'BaudRateConfigID',
        type: 'uint16',
        direction: 'in',
        description: '波特率配置标识符，引用预定义的波特率配置',
      },
    ],
    returnType: 'Std_ReturnType',
