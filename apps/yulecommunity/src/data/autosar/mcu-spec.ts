...
export const MCU_APIS: AutosarApi[] = [
  {
    ...
    description:
      '初始化微控制器模块，配置系统时钟源、PLL 倍频系数、分频器和复位参数。必须在任何外设初始化和时钟配置前调用。',
    params: [
      {
        name: 'ConfigPtr',
        type: 'const Mcu_ConfigType*',
        direction: 'in',
        description: 'Mcu 配置指针，包含时钟源选择和 PLL 配置',
      },
    ],
    returnType: 'void',
...
void SystemInit(void) {
  ...
  },
  {
    ...
    brief: '激活指定时钟配置',
    description:
      '根据配置 ID 激活预定义的时钟设置，包括 PLL 锁定目标频率、分频系数等。在切换时钟时需要等待 PLL 锁定。',
    params: [
      {
        name: 'ClockSetting',
        type: 'Mcu_ClockType',
        direction: 'in',
        description: '时钟配置 ID，引用预定义的时钟设置',
      },
    ],
...
void Mcu_SwitchClock(void) {
  ...
  },
  {
    ...
    description:
      '在 PLL 锁定后，将 PLL 时钟分发到各外设总线模块。必须在 Mcu_InitClock 成功且 PLL 锁定后才能调用。',
    params: [],
    returnType: 'Std_ReturnType',
...
void Mcu_EnablePeripheralClocks(void) {
  ...
  },
  {
    ...
    brief: '获取 PLL 锁定状态',
    description:
      '返回 PLL 的当前锁定状态。在 Mcu_InitClock 后轮询此函数，直到返回 MCU_PLL_LOCKED 表示 PLL 已稳定。',
    params: [],
...
void Mcu_WaitPllLock(void) {
  ...
  },
  {
    ...
    brief: '设置微控制器功耗模式',
    description:
      '切换微控制器的功耗模式，支持 RUN（正常运行）、WAIT（外设运行、CPU 停止）和 STOP（深度睡眠）三种模式。用于系统低功耗管理。',
    params: [
      {
        name: 'McuMode',
        type: 'Mcu_ModeType',
        direction: 'in',
        description: '目标功耗模式：MCU_MODE_RUN / MCU_MODE_WAIT / MCU_MODE_STOP',
      },
    ],
    returnType: 'void',
    returnDescription: '无返回值',
...
void Mcu_EnterSleep(void) {
  ...
  },
  {
    ...
    brief: '获取最近一次复位原因',
    description:
      '返回最近一次系统复位的原因，如上电复位、外部复位引脚、看门狗复位、低电压检测等。用于系统诊断和错误分析。',
    params: [],
...
void Mcu_DiagnoseReset(void) {
  ...
  },
  {
    ...
    brief: '获取原始复位寄存器值',
    description:
      '直接读取复位状态寄存器的原始值，提供比 Mcu_GetResetReason 更详细的复位源信息，用于深入调试。',
    params: [],
