...
import {
  ...
  getRegistryStats,
  fetchRegistryList,
  fetchRegistryStats as fetchStats,
} from '../../data/autosar/registry-samples';
  ...
  return (
    <DevHubLayout title="模块仓库" backTo="/autosar">
      <Helmet>
        ...
        <meta
          name="description"
          content="社区共建的 AutoSAR BSW 模块模板仓库，支持搜索、筛选和一键导入 yuleASR 配置器。"
        />
      </Helmet>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 px-3 py-2.5 text-sm rounded-lg bg-card border border-border">
              ...
              <select
                value={filters.sort}
                onChange={e =>
                  setFilters(prev => ({ ...prev, sort: e.target.value as RegistryFilter['sort'] }))
                }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {filteredModules.length > 0 ? (
          filteredModules.map((mod, i) => <ModuleCard key={mod.id} module={mod} index={i} />)
        ) : (
