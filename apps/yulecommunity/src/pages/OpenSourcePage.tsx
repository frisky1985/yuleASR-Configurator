...
export function OpenSourcePage() {
  ...
  const matchedReposCount = useMemo(() => {
    return modules.reduce((sum, g) => sum + g.items.filter(i => i.repo !== undefined).length, 0);
  }, [modules]);
  ...
  return (
    <div className="min-h-screen pt-16">
      <Helmet>
        ...
        <meta
          name="description"
          content="基于 AutoSAR Classic Platform 4.x 标准，为全球主流芯片公司的处理器构建完整的 BSW 栈。从底层驱动到应用组件，全部开源，永久免费。"
        />
      </Helmet>
      {/* Hero */}
                ...
                {mod.items.map(item => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Link to={`/opensource/${item.name}`} className="block">
      ...
      <section className="py-16 bg-muted/30 border-t border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">
              可视化展示 YuleTech BSW 各模块间的依赖关系，从 MCAL 底层驱动到 ASW
              应用软件的完整调用链。 点击节点查看详细信息，使用左侧控制面板筛选不同层级。
            </p>
          </div>
          <DependencyGraph />
        </div>
      ...
      <section className="py-20 bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--accent))]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-white/80 mb-8 max-w-2xl mx-auto">
            无论你是经验丰富的汽车软件工程师，还是刚入门的爱好者， YuleTech
            社区都欢迎你的贡献。提交代码、报告问题、完善文档， 与我们一起打造中国最优秀的 AutoSAR
            开源生态。
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
