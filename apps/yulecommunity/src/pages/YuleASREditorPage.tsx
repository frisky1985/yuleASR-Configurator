...
export function YuleASREditorPage() {
          ...
          },
          {
            ...
            parameters: [
              ...
              },
              {
                ...
                type: 'enum',
                value: 'aggressive',
                options: ['normal', 'aggressive', 'low_bitrate'],
                description: 'VAD模式',
              },
            ],
          },
  ...
  const handleValidate = useCallback(async () => {
        ...
        if (param.type === 'integer' || param.type === 'float') {
          ...
          if (param.min !== undefined && val < param.min) {
            errors.push({
              ...
              message: `${param.name} 值 ${val} 低于最小值 ${param.min}`,
              severity: 'error',
            });
          }
          if (param.max !== undefined && val > param.max) {
            errors.push({
              path: `${module.name}.${param.name}`,
              message: `${param.name} 值 ${val} 超过最大值 ${param.max}`,
              severity: 'error',
            });
          }
        }
    ...
    if (enabledCount < 3) {
      warnings.push({
        path: 'config.modules',
        message: `仅启用了 ${enabledCount} 个模块，建议启用更多模块以获得完整功能`,
        severity: 'warning',
      });
    }
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-4 h-4 rounded border-border"
                      />
                      <label htmlFor="enableLTO" className="text-sm text-foreground">
                        启用链接时优化 (LTO)
                      </label>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-4 h-4 rounded border-border"
                      />
                      <label htmlFor="stripSymbols" className="text-sm text-foreground">
                        剥离调试符号
                      </label>
                    </div>
                  </div>
