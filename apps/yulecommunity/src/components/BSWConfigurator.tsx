...
const BSW_MODULES: BSWModule[] = [
  // MCAL 层
  {
    ...
    configurable: true,
    dependencies: [],
    defaultConfig: { ClockSource: 'PLL', SysTick: true },
  },
  {
    id: 'port',
    name: 'Port',
    category: 'MCAL',
    description: 'GPIO 端口驱动',
    icon: <Network className="w-4 h-4" />,
    configurable: true,
    dependencies: ['mcu'],
    defaultConfig: { PinCount: 32 },
  },
  {
    id: 'dio',
    name: 'Dio',
    category: 'MCAL',
    description: '数字 IO 驱动',
  ...
  },
  {
    ...
    name: 'Can',
    category: 'MCAL',
    description: 'CAN 控制器驱动',
    icon: <Network className="w-4 h-4" />,
    configurable: true,
    dependencies: ['mcu'],
    defaultConfig: { Baudrate: 500000, ControllerCount: 2 },
  },
  {
    id: 'spi',
    name: 'Spi',
    category: 'MCAL',
    description: 'SPI 串行外设驱动',
    icon: <Settings className="w-4 h-4" />,
    configurable: true,
    dependencies: ['mcu'],
    defaultConfig: { ChannelCount: 4 },
  },
  {
    id: 'adc',
    name: 'Adc',
    ...
    configurable: true,
    dependencies: ['mcu'],
    defaultConfig: { Resolution: 12, ChannelCount: 8 },
  },
  {
    id: 'pwm',
    name: 'Pwm',
    category: 'MCAL',
    description: 'PWM 脉冲输出驱动',
    icon: <Settings className="w-4 h-4" />,
    configurable: true,
    dependencies: ['mcu'],
    defaultConfig: { ChannelCount: 4, Frequency: 1000 },
  },
  ...
  // ECUAL 层
  {
    ...
    configurable: true,
    dependencies: ['can'],
    defaultConfig: { HohCount: 4, TxConfirmation: true },
  },
  {
    id: 'cantp',
    name: 'CanTp',
    ...
    configurable: true,
    dependencies: ['canif'],
    defaultConfig: { ChannelCount: 2 },
  },
  {
    id: 'iohwab',
    name: 'IoHwAb',
    category: 'ECUAL',
    description: 'IO 硬件抽象层',
    icon: <Settings className="w-4 h-4" />,
    configurable: true,
  ...
  },
  {
    ...
    name: 'Fee',
    category: 'ECUAL',
    description: 'Flash EEPROM 仿真',
    icon: <Database className="w-4 h-4" />,
    configurable: true,
    dependencies: ['memif'],
    defaultConfig: { BlockCount: 32 },
  },
  {
    id: 'ea',
    name: 'Ea',
    category: 'ECUAL',
    description: 'EEPROM 抽象层',
    icon: <Database className="w-4 h-4" />,
    configurable: true,
    dependencies: ['memif'],
    defaultConfig: {},
  ...
  // Service 层
  {
    ...
    configurable: true,
    dependencies: ['canif'],
    defaultConfig: { SignalCount: 64, PduCount: 16 },
  },
  {
    id: 'pdur',
    name: 'PduR',
    ...
    icon: <Network className="w-4 h-4" />,
    configurable: true,
    dependencies: ['com', 'canif'],
    defaultConfig: { RoutingPathCount: 16 },
  },
  {
    id: 'nvm',
    name: 'NvM',
    ...
    configurable: true,
    dependencies: ['memif'],
    defaultConfig: { BlockCount: 64, WriteCycle: 100000 },
  },
  {
    id: 'dcm',
    name: 'Dcm',
    ...
    configurable: true,
    dependencies: ['pdur'],
    defaultConfig: { DidCount: 32, RidCount: 8 },
  },
  {
    id: 'dem',
    name: 'Dem',
    ...
    configurable: true,
    dependencies: ['dcm'],
    defaultConfig: { EventCount: 64, DTCCount: 32 },
  },
  ...
  // RTE 层
  {
    ...
    configurable: true,
    dependencies: ['com', 'os'],
    defaultConfig: { ComponentCount: 16, InterfaceCount: 32 },
  },
];
...
const MODULE_CONFIGS: Record<string, ConfigParam[]> = {
  ...
  pdur: [
    {
      name: 'RoutingPathCount',
      type: 'number',
      label: '路由路径数',
      description: 'PDU 路由路径总数',
      defaultValue: 16,
      min: 1,
      max: 256,
    },
...
export function BSWConfigurator() {
  ...
  // 按类别分组的模块
  const groupedModules = BSW_MODULES.reduce(
    (acc, module) => {
        ...
        <div className="w-56 border-r border-border">
          ...
          <div className="overflow-y-auto" style={{ maxHeight: '450px' }}>
            {selectedModules.length === 0 ? (
              <div className="p-4 text-center text-sm text-muted-foreground">尚未选择模块</div>
            ) : (
                ...
                return (
                  <div>
                    <div className="flex items-center gap-3 mb-4 pb-3 border-b border-border">
                      <div className="p-2 rounded-lg bg-primary/10">{module?.icon}</div>
                      <div>
                          ...
                          <div key={param.name} className="p-3 rounded-lg bg-muted/30">
                            ...
                            {param.type === 'select' && (
                              <select
                                ...
                                onChange={e =>
                                  updateConfig(activeModule, param.name, e.target.value)
                                }
                            ...
                            {param.type === 'number' && (
                              <div className="flex items-center gap-3">
                                <input
                                  ...
                                  value={currentConfig[param.name]}
                                  onChange={e =>
                                    updateConfig(activeModule, param.name, parseInt(e.target.value))
                                  }
                                  className="flex-1"
                                />
                                <input
