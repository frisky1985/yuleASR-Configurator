...
import { GitGraph, ZoomIn, ZoomOut, Maximize2, X, Download } from 'lucide-react';
...
export function DependencyGraph() {
  ...
  const [visibleCategories, setVisibleCategories] = useState<Set<string>>(
    new Set(['MCAL', 'ECUAL', 'Service', 'RTE', 'APP'])
  );
          ...
          >
            ...
            <g transform={`translate(${translate.x}, ${translate.y}) scale(${scale})`}>
              ...
              {edges.map((edge, index) => {
                ...
                if (
                  !visibleCategories.has(fromNode.category) ||
                  !visibleCategories.has(toNode.category)
                )
                  return null;
              ...
              {visibleModules.map(module => {
                ...
                return (
                  ...
                  >
                    {/* 外发光 */}
                    {hasHighlight && (
                      <circle r="28" fill={CATEGORY_COLORS[module.category]} opacity="0.2" />
                    )}
                ...
                return (
                  <>
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className="w-3 h-3 rounded-full"
                        style={{ backgroundColor: CATEGORY_COLORS[module.category] }}
                      />
                    ...
                    <div className="mt-2 flex items-center gap-2 text-xs">
                      <span className="px-1.5 py-0.5 rounded bg-muted">
                        完成度: {module.completion}%
                      </span>
                      <span
                        className="px-1.5 py-0.5 rounded"
                        style={{ backgroundColor: STATUS_COLORS[module.status], color: '#fff' }}
                      >
                        {module.status}
                      </span>
                    </div>
