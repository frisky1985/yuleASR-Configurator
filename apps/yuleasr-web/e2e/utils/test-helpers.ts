...
export async function waitForToast(page: Page, text?: string, timeout = 5000) {
  const toast = text ? page.getByText(text) : page.locator('[class*="toast"], [role="status"]');
 ...
 */
export async function waitForText(locator: Locator, text: string | RegExp, timeout = 5000) {
 ...
 */
export async function retry<T>(action: () => Promise<T>, maxRetries = 3, delay = 500): Promise<T> {
 ...
 */
export async function fillAndWait(locator: Locator, value: string, waitMs = 500) {
