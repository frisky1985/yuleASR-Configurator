...
export class DashboardPage {
  ...
  constructor(page: Page) {
    ...
    this.quickActions = page
      .locator('section, div')
      .filter({ hasText: /快速操作|Quick/i })
      .first();
    this.openExistingButton = page
      .locator('button')
      .filter({ hasText: /打开现有|Open/i })
      .first();
    this.importConfigButton = page
      .locator('button')
      .filter({ hasText: /导入|Import/i })
      .first();
    this.templatesButton = page
      .locator('button')
      .filter({ hasText: /模板|Template/i })
      .first();

    // Config list
    this.configListContainer = page
      .locator('section, div')
      .filter({ hasText: /最近配置|Recent/i })
      .first();
    this.configListHeader = page
      .locator('h2, h3')
      .filter({ hasText: /最近配置|Recent/i })
      .first();
    ...
    this.configNameInput = page
      .locator('input[placeholder*="我的配置"], input[placeholder*="配置名称"]')
      .first();
    this.configDescInput = page
      .locator('textarea[placeholder*="可选描述"], textarea[placeholder*="description"]')
      .first();
