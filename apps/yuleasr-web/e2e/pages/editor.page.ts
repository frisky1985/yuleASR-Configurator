...
export class EditorPage {
  ...
  constructor(page: Page) {
    ...
    this.layerHeaders = page
      .locator('div[class*="cursor-pointer"]')
      .filter({ hasText: /MCAL|ECUAL|Service|RTE|ASW/ });
    this.moduleItems = page
      .locator('div[class*="cursor-pointer"]')
      .filter({ hasText: /Driver|Unit|Library|\sModule/i });

    // Parameter Editor — center column, appears after clicking a container
    this.parameterEditor = page
      .locator('div')
      .filter({
        has: page.getByRole('heading', { name: /Configuration Parameters|Configuration$/i }),
      })
      .first();

    // Validation Panel — right sidebar
    this.validationPanel = page
      .locator('div')
      .filter({ has: page.getByRole('heading', { name: /^Validation$/i }) })
      .first();
  ...
  async isValidated(): Promise<boolean> {
    return await this.page
      .getByText(/Validation Passed|Issue|No validation/)
      .isVisible()
      .catch(() => false);
  }
