...
export class ParameterValidator {
  ...
  private initializeDefaultRules() {
    ...
    this.rules.set('McuClockSettingConfig.McuClockReferencePointFrequency', [
      ...
      {
        type: 'range',
        message: 'Frequency must be between 1 and 1000 MHz',
    ...
    this.rules.set('CanControllerBaudrateConfig.CanControllerBaudRate', [
      ...
      {
        type: 'enum',
        message: 'Standard baudrates: 125000, 250000, 500000, 1000000',
        params: { values: [125000, 250000, 500000, 1000000] },
      },
    ]);
    ...
    this.rules.set('PwmChannelConfig.PwmChannelFrequency', [
      {
        type: 'range',
        message: 'PWM frequency must be between 0.1 and 10000 Hz',
        params: { min: 0.1, max: 10000 },
      },
    ]);
    ...
    this.rules.set('AdcChannelConfig.AdcChannelResolution', [
      {
        type: 'enum',
        message: 'Valid resolutions: 8, 10, 12 bits',
        params: { values: [8, 10, 12] },
      },
    ]);
    ...
    this.rules.set('GptChannelConfig.GptChannelTickFrequency', [
      {
        type: 'range',
        message: 'Tick frequency must be between 1 and 1000000 Hz',
    ...
    this.rules.set('PortPinConfig.PortPinMode', [
      {
        type: 'enum',
        message: 'Valid modes: INPUT, OUTPUT, ANALOG, ALTERNATE',
        params: { values: ['INPUT', 'OUTPUT', 'ANALOG', 'ALTERNATE'] },
      },
    ]);
    ...
    this.rules.set('OsTaskConfig.OsTaskPriority', [
      {
        type: 'range',
        message: 'Priority must be between 0 and 255',
    ...
    this.rules.set('OsTaskConfig.OsTaskStackSize', [
      {
        type: 'range',
        message: 'Stack size must be between 64 and 65536 bytes',
        params: { min: 64, max: 65536 },
      },
    ]);
  }
   ...
   */
  validateRelationships(parameters: Record<string, unknown>): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    const canSamplePoint = parameters[
      'CanControllerBaudrateConfig.CanControllerSamplePoint'
    ] as number;
