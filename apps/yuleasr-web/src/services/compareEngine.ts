...
function compareModules(
  moduleA: ConfigModule | undefined,
  moduleB: ConfigModule | undefined
): ModuleDiff {
  if (moduleA && moduleB) {
...
function compareParameters(
  ...
  const allIds = new Set([...paramsA.map(p => p.id), ...paramsB.map(p => p.id)]);
          ...
          if (containerA && containerB) {
            ...
            if (containerA.subContainers || containerB.subContainers) {
              ...
              for (const subId of Array.from(allSubIds)) {
                ...
                if (subA && subB) {
                  const subPath = buildContainerPath(
                    moduleA.name,
                    `${containerA.name}.${subA.name}`
                  );
