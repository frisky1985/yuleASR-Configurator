...
describe('T10 required 依赖必须存在（老板钦定：Can 依赖时钟未配置 → 报错）', () => {
  it('Can 配置但 Mcu 时钟未配置 → CROSS_REF_REQUIRED 报错', () => {
    ...
    console.log(`[T10] 全部错误 ${errors.length} 条: ${errors.map(e => e.code + ':' + e.message).join(' | ')}`);
    const req = errors.filter(e => e.code === 'CROSS_REF_REQUIRED');
    console.log(`[T10] Can 无 Mcu → ${req.length} 条 required 错误: ${req.map(e => e.message).join(' | ')}`);
  ...
  it('CanTp 配置但 Can 未配置 → 报错', () => {
    const v = makeValidator();
    const configs = [
      { module: 'CanTp', parameters: { CANTP_NUM_CHANNELS: 1 } },
    ];
    const errors = v.validate(configs as any);
    const req = errors.filter(e => e.code === 'CROSS_REF_REQUIRED');
  ...
  it('required 缺失是 error 级（不是 warning）', () => {
    ...
    const errors = v.validate([
      { module: 'Can', parameters: { CAN_NUM_CONTROLLERS: 2 } },
    ] as any);
    const req = errors.filter(e => e.code === 'CROSS_REF_REQUIRED');
    expect(req.length).toBeGreaterThan(0);
