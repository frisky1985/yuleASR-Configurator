 ...
 */
function expectModuleRoundtrip(original: ConfigModule, restored: ConfigModule | undefined) {
...
function expectContainersEqual(original: ConfigContainer[], restored: ConfigContainer[]) {
...
function expectParamsEqual(original: ConfigParameter[], restored: ConfigParameter[]) {
  expect(restored.length).toBe(original.length);
...
describe('ARXML Round-Trip', () => {
  ...
  it('should round-trip Port module with multiple PortPin instances', () => {
    ...
    expect(portPins[0].parameters.find(p => p.name === 'portPinDirection')?.value).toBe(
      'PORT_PIN_IN'
    );

    // Second PortPin
    expect(portPins[1].parameters.find(p => p.name === 'portPinId')?.value).toBe(1);
    expect(portPins[1].parameters.find(p => p.name === 'portPinDirection')?.value).toBe(
      'PORT_PIN_OUT'
    );
  });
  ...
  it('should round-trip all 37 BSW modules without errors', () => {
    const modules: ConfigModule[] = ALL_37_MODULES.map((name, i) => {
      const layers: Array<'MCAL' | 'ECUAL' | 'Service' | 'OS'> = ['MCAL', 'ECUAL', 'Service', 'OS'];
...
<AUTOSAR xmlns="http://autosar.org/schema/r4.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  it('should round-trip nested sub-containers with params at every level', () => {
    ...
    expect(
      r.containers[0].subContainers![0].parameters.find(p => p.name === 'level2Param')?.value
    ).toBe(2);
    expect(r.containers[0].subContainers![0].subContainers![0].name).toBe('Level3');
    expect(
      r.containers[0].subContainers![0].subContainers![0].parameters.find(
        p => p.name === 'level3Param'
      )?.value
    ).toBe(3);
    expect(r.containers[0].subContainers![0].subContainers![0].subContainers![0].name).toBe(
      'Level4'
    );
    expect(
      r.containers[0].subContainers![0].subContainers![0].subContainers![0].parameters.find(
        p => p.name === 'level4Param'
      )?.value
    ).toBe(4);
  });
