...
export function useModuleLayout(
  ...
  const calculatePositions = useCallback((): Map<string, { x: number; y: number }> => {
    ...
    LAYER_ORDER.forEach((layer, layerIndex) => {
      ...
      const totalWidth =
        layerModules.length * opts.nodeWidth! + (layerModules.length - 1) * opts.horizontalSpacing!;
...
export function useDependencyHighlight(edges: Edge[], selectedNodeId: string | null): Edge[] {
  return useMemo(() => {
    ...
    return edges.map(edge => ({
      ...
      style: {
        ...
        stroke: connectedEdgeIds.has(edge.id)
          ? '#3b82f6'
          : (edge.style?.stroke as string) || '#94a3b8',
