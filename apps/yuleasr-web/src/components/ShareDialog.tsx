...
export function ShareDialog({ isOpen, onClose, config }: ShareDialogProps) {
  ...
  const [publishState, setPublishState] = useState<'idle' | 'loading' | 'success' | 'error'>(
    'idle'
  );
  ...
  const handleCopyLink = useCallback(async () => {
    ...
    try {
      ...
      const result = await api.put<{ shareToken: string }>(`/api/configs/${config.id}/share`);
      ...
      setLinkState('error');
      setLinkError(err instanceof Error ? err.message : '生成分享链接失败，请重试');
    }
  }, [config.id]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-app-bg-primary rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-app-border-primary shrink-0">
          <h2 className="text-lg font-semibold text-app-text-primary">分享配置</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
            ...
            {!isAuthenticated ? (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-app-text-primary">分享到社区</h3>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-green-700">发布成功 🎉</h3>
          ...
          >
            {!isAuthenticated ? (
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-app-text-primary">分享到 Gallery</h3>
                  <p className="text-sm text-app-text-secondary mt-1">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setPublishState('error');
                        setPublishError(err instanceof Error ? err.message : '分享失败，请重试');
                      }
                    }}
                    disabled={publishState === 'loading'}
                    className={cn(
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-medium text-app-text-primary">下载到本地</h3>
                <p className="text-sm text-app-text-secondary mt-1">
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-medium text-app-text-primary">复制分享链接</h3>
                <p className="text-sm text-app-text-secondary mt-1">
