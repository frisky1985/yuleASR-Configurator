...
export function registerCommands(
    ...
    // Command: New Module
    const newModuleCmd = vscode.commands.registerCommand(
        'yuleasr.newModule',
        async () => {
    ...
    vscode.workspace.onDidSaveTextDocument(async (document) => {
        ...
        if (config.get<boolean>('validateOnSave', true)) {
            if (document.fileName.endsWith('.yule.json') || 
...
async function generateCodeForConfig(filePath: string, moduleName?: string): Promise<void> {
    ...
    vscode.window.showInformationMessage(`Code generated for ${moduleName || path.basename(filePath)}`);
}
